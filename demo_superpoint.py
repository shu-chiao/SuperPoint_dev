...
if __name__ == '__main__':
  ...
  out = np.zeros((opt.H*opt.display_scale, opt.W*opt.display_scale, 3), dtype=np.uint8)
  image_plot = plt.imshow(cv2.cvtColor(out, cv2.COLOR_BGR2RGB))
  plt.draw()
  while True:
    ...
    if not opt.no_display:
      ...
      #   break
      image_plot.set_data(cv2.cvtColor(out, cv2.COLOR_BGR2RGB))
      plt.draw()
      plt.pause(0.01)
